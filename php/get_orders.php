<?php
    if($_GET["request"]) {
        ...
        //get orders buy
        $query_buy = mysqli_query($connect, "SELECT `id`, `date`, `time`, `name`, `quantity`, `price`, `type`  FROM `orders`  WHERE `type` = 'buy' ORDER BY `price`");
        ...
        //get orders sell
        $query_sell = mysqli_query($connect, "SELECT `id`, `date`, `time`, `name`, `quantity`, `price`, `type`  FROM `orders` WHERE `type` = 'sell' ORDER BY `price`");
